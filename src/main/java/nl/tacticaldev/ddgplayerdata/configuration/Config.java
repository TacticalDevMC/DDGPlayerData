package nl.tacticaldev.ddgplayerdata.configuration;

import nl.tacticaldev.ddgplayerdata.DDGPlayerData;
import nl.tacticaldev.ddgplayerdata.configuration.exceptions.InvalidWorldException;
import org.bukkit.*;
...
public class Config {
    ...
    public Config(String name, boolean isDef) {
        this(DDGPlayerData.getInstance().getDataFolder(), name, isDef);
    }

    public Config(String name) {
        this(DDGPlayerData.getInstance().getDataFolder(), name, true);
    }
    ...
    public boolean reload() {
        if (!this.folder.exists()) {
            try {
                if (this.folder.mkdir()) {
                    DDGPlayerData.getInstance().getLogger().log(Level.INFO, "- Created a new configuration file!");
                } else {
                    DDGPlayerData.getInstance().getLogger().log(Level.INFO, "- §cUnable to create folder " + this.folder.getName());
                }
            } catch (Exception e) {
                DDGPlayerData.getInstance().getLogger().log(Level.INFO, "Failed to reload " + this.fileName + " config.");
                return false;
            }
        }
        ...
        if (this.def) {
            InputStream defaultConfigStream = DDGPlayerData.getInstance().getResource(this.fileName);
...
//    }
    ...
    public boolean save() {
        ...
        try {
            ...
            DDGPlayerData.getInstance().getLogger().log(Level.INFO, ("Failed to save config to " + this.configFile.getName()));
            e.printStackTrace();
            return false;
        }
    }
    ...
    private void saveDefaultConfig() {
        if (this.configFile == null)
            this.configFile = new File(DDGPlayerData.getInstance().getDataFolder(), this.fileName);

        if (!this.configFile.exists()) DDGPlayerData.getInstance().saveResource(this.fileName, false);
    }
}
